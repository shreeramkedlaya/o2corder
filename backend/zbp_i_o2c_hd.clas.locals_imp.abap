CLASS lhc_OrderHeader DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS setOpenStatus FOR DETERMINE ON SAVE
      IMPORTING keys FOR OrderHeader~setOpenStatus.
...
CLASS lhc_OrderHeader IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    READ ENTITIES OF zi_o2c_hd IN LOCAL MODE
      ENTITY OrderHeader
        FIELDS ( overall_status credit_check_status ) WITH CORRESPONDING #( keys )
      RESULT DATA(lt_orders).

    result = VALUE #( FOR ls_order IN lt_orders
                      ( %tky = ls_order-%tky

                        " Edit and Delete always enabled for sandbox testing
  ...
  METHOD createDelivery.
    ...
    LOOP AT lt_orders INTO DATA(ls_order).
      ...
      " 1. Read the Sales Order Items associated with this order
      READ ENTITIES OF zi_o2c_hd IN LOCAL MODE
        ENTITY OrderHeader BY \_Items
        ALL FIELDS WITH VALUE #( ( %tky = ls_order-%tky ) )
        RESULT DATA(lt_order_items).

      " 2. Create the Delivery Header AND its Delivery Items via inter-entity EML
      MODIFY ENTITIES OF zi_o2c_del_hd
        ENTITY DeliveryHeader
          ...
          WITH VALUE #( ( %cid            = |cid_{ ls_order-order_id }|
                          ...
                          delivery_status = 'P' ) )
          " Create the child items (excluding delivery_item as it auto-numbers)
          CREATE BY \_DeliveryItems
          FIELDS ( order_item_position material_id quantity_ordered quantity_delivered unit_of_measure )
          WITH VALUE #( ( %cid_ref = |cid_{ ls_order-order_id }|
                          %target  = VALUE #( FOR ls_item IN lt_order_items INDEX INTO i (
                                                %cid                = |cid_it_{ i }|
                                                order_item_position = ls_item-item_position
                                                material_id         = ls_item-material_id
                                                quantity_ordered    = ls_item-quantity
                                                quantity_delivered  = ls_item-quantity " Assuming full delivery initially
                                                unit_of_measure     = ls_item-unit_of_measure
                                              ) ) ) )
        MAPPED DATA(ls_mapped)  FAILED DATA(ls_failed)  REPORTED DATA(ls_reported).

      " 3. Advance Sales Order status to 'F' (In Fulfilment)
      MODIFY ENTITIES OF zi_o2c_hd IN LOCAL MODE
        ENTITY OrderHeader UPDATE FIELDS ( overall_status )
        WITH VALUE #( ( %tky = ls_order-%tky  overall_status = 'F' ) ).

      " 4. Extract the newly generated ID from the mapped table
      DATA lv_new_del_id TYPE ztab_o2c_del_hd-delivery_id.
      IF line_exists( ls_mapped-deliveryheader[ 1 ] ).
        lv_new_del_id = ls_mapped-deliveryheader[ 1 ]-delivery_id.
      ENDIF.
